...
class DisplayClient:
    ...
    def get_embed_html(self, snippet_id: str) -> str | None:
        """Fetch the self-contained static HTML for a snippet.

        Used to embed the visualization inline in Claude Desktop via
        ``iframe.srcdoc``. Returns ``None`` (with a warning log) on failure
        so the caller can degrade gracefully to the URL-only flow.
        """
        try:
            ...
            if response.status_code != 200:
                logger.warning(
                    "Embed render failed (HTTP %s) for snippet %s",
                    response.status_code, snippet_id,
                )
                return None
            content_type = response.headers.get("Content-Type", "")
            if "text/html" not in content_type:
                logger.warning("Embed render returned non-HTML content-type: %s", content_type)
                return None
